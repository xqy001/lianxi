...
class Simple extends IController
{
	...
	public function modify(){
		$user_id=IReq::get('id');
		// 使用IMOdel类更新iwebshop_member表中的status字段值，将其修改为1，这样就实现了账户激活，用户就可以使用此账号登录系统了
...
// 新增 开始  调用百度地图服务端接口 将地址转换成经纬度
$url="http://api.map.baidu.com/geocoder/v2/?address=$address&output=json&ak=fQuIOSXA4ClY1fCQ7avCLAEws1tqMOeG&callback=showLocation";
$ch=curl_init();
curl_setopt($ch, CURLOPT_URL, $url);
curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
$str=curl_exec($ch);
echo $str;die;
// string(159) "showLocation&&showLocation({"status":0,"result":{"location":{"lng":112.56816009424296,"lat":37.741843645560859},"precise":1,"confidence":90,"level":"医疗"}})" 
// 根据分析，我们需要获取左圆括号和右圆括号之前文本，这些文本是正常的json字符串
$leftPos=strpos($str,'(');		// 查询左括号“（”，在字符串中首次出现的位置
$rightPos=strpos($str,')');		// 查询右括号“）”，在字符串中首次出现的位置
$str1=substr($str,$leftPos+1,$rightPos-$leftPos-1);		// 获取左右括号之间的位置
$data=json_decode($str1,true);					// 将json字符串转换成php数组
$lng=$data['result']['location']['lng'];		// 获取经度
$lat=$data['result']['location']['lat'];		// 获取纬度
$sellerRow['lng']=$lng;		// 存放经度
$sellerRow['lat']=$lat;		// 存放纬度
// 新增 结束
